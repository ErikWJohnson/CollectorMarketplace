...
document.addEventListener('input', event => { if (event.target.closest('.listing-form')) saveListingDraft(event.target.closest('.listing-form')); if (event.target.closest('.fee-calculator')) updateFeeSummary(); if (event.target.matches('#shipping-zip')) { clearTimeout(checkoutAddressTimer); checkoutAddressTimer = setTimeout(() => calculateCheckoutDeliveryFromAddress(event.target), 500); } if (event.target.closest('.trade-builder-form')) { if (event.target.matches('[name="senderCashAmount"], [name="receiverCashAmount"]')) syncTradeCashInput(event.target); updateTradeSelectionCounts(); } if (event.target.matches('[name="imageUrls"]')) syncListingPreview(); });
