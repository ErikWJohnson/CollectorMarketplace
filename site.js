...
async function openProfile(id) { if (!id) return; if (session?.user?.id === id) return openAccountPanel(); const profile = await api(`/user/${id}`); const following = Boolean(session?.user?.following?.includes(id)); const profileTags = profile.profileTags || []; const listingsMarkup = profile.activeListings.length ? profile.activeListings.map(item => `<button type="button" class="profile-listing-card" data-detail="${safe(item.id)}"><img src="${safe(item.image || item.images?.[0] || '')}" alt="${safe(item.title)}"><b>${safe(item.title)}</b><span>${money(item.price)}</span><small>${safe(formatListingDate(item.createdAt))}</small></button>`).join('') : '<p class="profile-empty">This collector has no active listings right now.</p>'; const avatar = accountAvatar(profile, 'account-avatar'); openModal(`@${safe(profile.username)}`, `${safe(profile.bio || 'Collector profile')} · Reputation ${profile.reputation || 0}`, `<section class="profile-workspace"><header class="profile-hero">${avatar}<div><p class="profile-kicker">COLLECTOR PROFILE</p><h3>@${safe(profile.username)}</h3><span>${safe(profile.bio || 'A collector building their marketplace presence.')}</span></div>${session ? `<button type="button" data-follow="${profile.id}">${following ? 'Following' : 'Follow'}</button>` : ''}</header><aside class="profile-summary"><h4>Collector overview</h4><div class="profile-metrics"><span><b>${profile.activeListings.length}</b>Active posts</span><span><b>${profile.tradeHistory.length}</b>Completed trades</span><span><b>${Number(profile.reputation || 0)}</b>Reputation</span><span><b>${following ? '✓' : '—'}</b>Connection</span></div><h4>Discovery tags</h4><div class="profile-tag-list">${profileTags.length ? profileTags.map(tag => `<button type="button" data-tag="${safe(tag)}">#${safe(tag)}</button>`).join('') : '<span class="profile-empty">No profile tags yet.</span>'}</div></aside><section class="profile-listings"><h4>Active listings <small>${profile.activeListings.length}</small></h4><div class="profile-listing-grid">${listingsMarkup}</div></section><footer class="profile-footer"><button type="button" class="entity-primary" data-community="accounts" data-community-id="${profile.id}" data-community-label="@${safe(profile.username)}">Open discussion</button>${session ? `<button type="button" class="entity-primary" data-collector-chat="${profile.id}">Message collector</button>` : ''}</footer></section>`); modal.classList.add('profile-dialog'); document.body.classList.add('chat-open'); }
