...
document.addEventListener('submit', async event => { const form = event.target; const appSection = form.closest('.app-section-page'); if (form.closest('#modal') || appSection) { event.preventDefault(); const showError = error => { const host = appSection?.querySelector('.app-section-content') || modalContent; host.querySelector('.form-submit-error')?.remove(); host.insertAdjacentHTML('afterbegin', `<p class="modal-copy form-submit-error" role="alert">${safe(error.message)}</p>`); };
  try {
    ...
    const trade = form.matches('.fee-calculator[data-type="trade"]'); const purchase = form.matches('.fee-calculator[data-type="purchase"]'); if (purchase) { if (!session) return openAuthPanel('login'); const submit = form.querySelector('.transaction-submit'); submit.disabled = true; submit.textContent = 'Opening PayPal Sandbox…'; const order = await api('/paypal/orders', { method: 'POST', body: JSON.stringify({ listingId: form.dataset.listing, shippingAddress: form.querySelector('#shipping-address').value, deliveryProvider: form.querySelector('#delivery-provider').value, deliveryMiles: form.querySelector('#delivery-miles').value, paymentMethod: form.querySelector('[name="paymentMethod"]:checked')?.value }) }); window.location.assign(order.approvalUrl); return; } modalContent.innerHTML = trade ? '<h2 class="modal-title">Trade offer sent.</h2><p class="modal-copy">Your offer is pending with the collector. A trade chat is now open for delivery details and follow-up.</p>' : '<h2 class="modal-title">Saved.</h2><p class="modal-copy">Your update was saved.</p>';
