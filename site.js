...
const setSearchScope = (scope, syncRoute = true) => {
  ...
  search.placeholder = searchScope === 'listings' ? 'Search collector finds' : searchScope === 'accounts' ? 'Search collector accounts' : searchScope === 'collectives' ? 'Search collector collectives' : searchScope === 'brands' ? 'Search collectible brands' : searchScope === 'couriers' ? 'Search delivery couriers' : 'Search public chatrooms';
  renderFeed();
...
const ensurePublicListingLocation = () => {
  ...
  if (!form || form.querySelector('[name="location"]')) return;
  const tagField = form.querySelector('.listing-tag-field');
  if (!tagField) return;
  const locationField = document.createElement('label');
  locationField.className = 'listing-location-field';
  locationField.innerHTML = 'Public item location <i>Required · city, region, country only</i><input required name="location" maxlength="120" autocomplete="address-level2" placeholder="e.g. Seattle, Washington, USA"><input type="hidden" name="locationLat"><input type="hidden" name="locationLng"><button type="button" class="location-capture" data-capture-location>Use my approximate location</button><small class="location-capture-note">US locations automatically become a searchable city/town tag. Exact coordinates are never shown publicly.</small>';
  tagField.before(locationField);
  const fulfillmentField = document.createElement('fieldset');
  fulfillmentField.className = 'listing-fulfillment-field';
  fulfillmentField.innerHTML = '<legend>Fulfillment <i>Required</i></legend><div><label><input required type="radio" name="fulfillment" value="pickup"> Pickup</label><label><input type="radio" name="fulfillment" value="pickup_delivery"> Pickup &amp; delivery</label></div><small>Pickup details are shared privately only after an order is confirmed.</small>';
  locationField.after(fulfillmentField);
  const auctionField = document.createElement('fieldset');
  auctionField.className = 'listing-fulfillment-field listing-auction-field';
  auctionField.innerHTML = '<legend>Where to list it <i>Required</i></legend><div><label><input required type="radio" name="listingMode" value="marketplace" checked> Marketplace</label><label><input type="radio" name="listingMode" value="auction_only"> Auction only</label><label><input type="radio" name="listingMode" value="marketplace_auction"> Marketplace + auction</label></div><section class="auction-listing-details" hidden><label>Starting bid<input name="auctionStartPrice" type="number" min="0" step="0.01" placeholder="0.00"></label><label>Auction length<select name="auctionDurationHours"><option value="24">1 day</option><option value="72" selected>3 days</option><option value="168">7 days</option><option value="336">14 days</option></select></label></section><small>Auction-only posts appear in the Auction House, while Marketplace + auction posts appear in both places.</small>';
  fulfillmentField.after(auctionField);
  const required = form.querySelector('.listing-publish-bar span');
  if (required) required.textContent = 'Required: title, tags, public item location, fulfillment, price, description, and at least one photo.';
};
new MutationObserver(ensurePublicListingLocation).observe(modalContent, { childList: true, subtree: true });
...
const deliveryCarriers = ['USPS', 'UPS', 'FedEx', 'DHL Express', 'Amazon Logistics', 'OnTrac', 'LaserShip', 'Veho', 'Roadie', 'DoorDash Drive', 'Uber Direct', 'Instacart Local Delivery', 'GoShare', 'uShip', 'Shippo', 'Pirate Ship', 'Canada Post', 'Purolator', 'Royal Mail', 'Evri', 'DPD', 'Australia Post', 'Local independent courier', 'Independent owner-operator', 'White-glove delivery', 'Freight / LTL carrier', 'Local pickup', 'Other courier'];
...
const card = item => {
  const state = stateFor(item.id); const owner = item.owner?.username || item.ownerName || 'collector'; const avatar = owner.slice(0, 1).toUpperCase();
...
const directoryCard = (item, type) => type === 'accounts' ? `<article class="search-directory-card" data-directory-page="accounts" data-directory-id="${safe(item.id)}"><b>@${safe(item.username)}</b><span>${safe(item.bio || 'Collector profile')}</span><small>${Number(item.activeListingCount || 0)} posts · ${Number(item.followingCount || 0)} following · ${Number(item.reputation || 0)} reputation${item.profileTags?.length ? ` · ${(item.profileTags || []).map(tag => '#' + safe(tag)).join(' ')}` : ''}</small><button type="button" data-community="accounts" data-community-id="${safe(item.id)}" data-community-label="@${safe(item.username)}">Open discussion</button></article>` : type === 'brands' ? `<article class="search-directory-card collective-card brand-card" data-directory-page="brands" data-directory-id="${safe(item.id)}"><b>${safe(item.name)}</b><span>${safe(item.description)}</span><small>${safe(item.category || 'Collector brand')}</small><button type="button" data-community="brands" data-community-id="${safe(item.id)}" data-community-label="${safe(item.name)}">Open discussion</button></article>` : type === 'couriers' ? `<article class="search-directory-card collective-card courier-card" data-directory-page="couriers" data-directory-id="${safe(item.id)}"><b>${safe(item.name)}</b><span>${safe(item.description)}</span><small>${safe(item.category || 'Courier')} · ${(item.tags || []).map(tag => '#' + safe(tag)).join(' ')}</small></article>` : type === 'chatrooms' ? `<article class="search-directory-card collective-card chatroom-card" data-directory-page="chatrooms" data-directory-id="${safe(item.id)}"><b>${safe(item.name)}</b><span>${safe(item.description)}</span><small>${Number(item.members || 0)} collectors · ${(item.tags || []).map(tag => '#' + safe(tag)).join(' ')}</small><button type="button" data-community="chatrooms" data-community-id="${safe(item.id)}" data-community-label="${safe(item.name)}">Open discussion</button><button type="button" data-chatroom="${safe(item.id)}" data-chatroom-label="${safe(item.name)}">Join voice room</button></article>` : `<article class="search-directory-card collective-card" data-directory-page="collectives" data-directory-id="${safe(item.id)}"><b>${safe(item.name)}</b><span>${safe(item.description)}</span><small>${Number(item.members || 0).toLocaleString()} members · ${Number(item.postCount || 0)} posts · ${(item.tags || []).map(tag => '#' + safe(tag)).join(' ')}</small><button type="button" data-community="collectives" data-community-id="${safe(item.id)}" data-community-label="${safe(item.name)}">Open discussion</button></article>`;
function renderFeed(reset = true) { if (searchScope !== 'listings') { const rows = filteredDirectory(); stream.innerHTML = rows.map(item => directoryCard(item, searchScope)).join('') || `<p class="load-state">No ${searchScope} match that search.</p>`; document.querySelector('#result-count').textContent = `${rows.length} ${searchScope}`; sentinel.textContent = 'Directory search complete.'; return; } const rows = filtered(); if (reset) page = 1; const visible = rows.slice(0, page * 4); stream.innerHTML = visible.map(card).join('') || '<p class="load-state">No collector finds match that search.</p>'; stream.querySelectorAll('.collector-head').forEach((header, index) => { header.dataset.profile = visible[index]?.ownerId || ''; header.tabIndex = 0; header.setAttribute('role', 'button'); header.setAttribute('aria-label', `Open ${visible[index]?.owner?.username || 'collector'} profile`); }); document.querySelector('#result-count').textContent = `${rows.length} listed`; sentinel.textContent = page * 4 < rows.length ? 'Scroll for more finds ↓' : 'You are all caught up.'; }
...
async function openProfile(id) { if (!id) return; if (session?.user?.id === id) return openAccountPanel(); const profile = await api(`/user/${id}`); const following = Boolean(session?.user?.following?.includes(id)); const posts = profile.activeListings.length ? profile.activeListings.map(item => `<p class="account-post">${safe(item.title)} · $${safe(item.price)}</p>`).join('') : '<p class="modal-copy">No active listings.</p>'; const profileTags = (profile.profileTags || []).length ? `<p class="profile-tag-list">${profile.profileTags.map(tag => `#${safe(tag)}`).join(' ')}</p>` : ''; openModal(`@${safe(profile.username)}`, `${safe(profile.bio || 'Collector profile')} · Reputation ${profile.reputation || 0}`, `<section class="account-panel"><div class="account-identity"><b>${safe(profile.avatar || profile.username.slice(0, 2).toUpperCase())}</b><div><strong>@${safe(profile.username)}</strong><span>${profile.activeListings.length} active posts · ${profile.tradeHistory.length} completed trades</span></div>${session ? `<button type="button" data-follow="${profile.id}">${following ? 'Following' : 'Follow'}</button>` : ''}</div>${profileTags}<div class="social-lists"><section><h3>Active posts <small>${profile.activeListings.length}</small></h3>${posts}</section></div></section><button type="button" class="auth-switch" data-community="accounts" data-community-id="${profile.id}" data-community-label="@${safe(profile.username)}">Open account discussion</button>${session ? `<button type="button" class="auth-switch" data-collector-chat="${profile.id}">Message collector</button>` : ''}`); }
...
async function openDirectoryPage(type, entityId) { if (type === 'accounts' && session?.user?.id === entityId) return openAccountPanel(); const source = type === 'accounts' ? accounts : type === 'brands' ? brands : type === 'couriers' ? couriers : type === 'chatrooms' ? chatrooms : collectives; let entity = source.find(item => item.id === entityId); if (type === 'accounts') entity = await api(`/user/${entityId}`); if (!entity) throw new Error('This directory page is unavailable.'); const name = type === 'accounts' ? `@${entity.username}` : entity.name; const tags = type === 'accounts' ? [...new Set([...(entity.profileTags || []), ...(entity.activeListings?.flatMap(listingTags) || [])])] : entity.tags || []; const stats = type === 'accounts' ? `${entity.activeListings?.length || entity.activeListingCount || 0} posts · ${entity.tradeHistory?.length || 0} completed trades` : type === 'couriers' ? `${entity.category || 'Courier'} · ${entity.description || ''}` : `${Number(entity.members || 0).toLocaleString()} members${entity.postCount !== undefined ? ` · ${entity.postCount} matching posts` : ''}`; const accountPosts = type === 'accounts' ? `<section class="entity-post-list"><h3>Listings</h3>${entity.activeListings?.length ? entity.activeListings.map(listing => `<button type="button" data-detail="${safe(listing.id)}">${safe(listing.title)}<span>${money(listing.price)}</span></button>`).join('') : '<p>No active listings yet.</p>'}</section>` : ''; const discussion = type === 'couriers' ? '' : `<button type="button" class="entity-primary" data-community="${safe(type)}" data-community-id="${safe(entity.id)}" data-community-label="${safe(name)}">Open discussion</button>`; const voice = type === 'chatrooms' ? `<button type="button" class="entity-primary" data-chatroom="${safe(entity.id)}" data-chatroom-label="${safe(name)}">Join live voice room</button>` : ''; openModal(name, safe(entity.description || entity.bio || 'Collector marketplace directory page.'), `<section class="entity-workspace"><header><span>${safe(type.slice(0, -1))}</span><h3>${safe(name)}</h3><p>${safe(stats)}</p></header><section class="entity-tags"><h3>Discovery tags</h3>${tags.length ? tags.map(tag => `<button type="button" data-tag="${safe(tag)}">#${safe(tag)}</button>`).join('') : '<p>No tags have been added yet.</p>'}</section>${accountPosts}<footer>${discussion}${voice}</footer></section>`); modal.classList.add('chat-dialog', 'entity-dialog'); document.body.classList.add('chat-open'); }
...
const tradePaymentOptions = ['Card (Square)', 'Apple Pay', 'Google Pay', 'Cash App Pay', 'Venmo', 'PayPal', 'ACH bank transfer'];
...
async function openDelivery(id) {
  ...
  const labels = { awaiting_seller_dispatch: 'Order placed', packed: 'Packed', picked_up: 'Picked up', in_transit: 'In transit', completed: 'Delivered', issue_reported: 'Issue reported' };
...
function feeCalculator(item, type) {
  ...
  const providerOptions = deliveryCarriers.map(provider => `<option value="${safe(provider)}">${safe(provider)}</option>`).join('');
  const paymentMethods = [['Card (Square)', 'Debit or credit card'], ['Apple Pay', 'Apple Wallet'], ['Google Pay', 'Google Wallet'], ['Cash App Pay', 'Cash App'], ['Venmo', 'Venmo balance or card'], ['PayPal', 'PayPal account'], ['ACH bank transfer', 'Bank account']];
  ...
  return `<form class="fee-calculator purchase-checkout" data-type="${type}" data-listing="${item.id}"><section class="purchase-fields"><header><span>1</span><div><b>Delivery details</b><small>Choose a provider, estimate the route, and confirm where this collector item should go.</small></div></header>${fields}${address}${memberships}${paymentChoices}</section><aside class="purchase-review"><header><span>2</span><div><b>Order review</b><small>Your order remains private until delivery is confirmed.</small></div></header><div id="fee-summary" class="fee-summary"></div>${agreement}<p class="policy-note">Placing an order reserves this listing and opens a private delivery thread with the seller. Your selected payment method is saved with the order; payment collection remains disabled until that provider’s production checkout is configured. Buyers pay taxes and shipping. Courier delivery pay is the greater of $8 or $0.20 per mile, plus documented packaging costs.</p><button type="submit" class="transaction-submit">${action}</button></aside></form>`;
}
function updateFeeSummary() {
  const box = document.querySelector('.fee-calculator'); if (!box) return;
  const type = box.dataset.type; const buyerValue = Number(document.querySelector('#buyer-value')?.value || 0); const sellerValue = Number(document.querySelector('#seller-value')?.value || buyerValue); const buyerRate = document.querySelector('#buyer-curator').checked ? .01 : .04; const sellerRate = document.querySelector('#seller-curator').checked ? .01 : .04; const tax = Number(document.querySelector('#tax')?.value || 0); const packaging = Number(document.querySelector('#packaging')?.value || 0); const miles = Number(document.querySelector('#delivery-miles')?.value || 0); const courier = Math.max(8, miles * .20) + packaging;
  const paymentMethod = document.querySelector('[name="paymentMethod"]:checked')?.value || 'Select a payment method';
  document.querySelector('#fee-summary').innerHTML = type === 'trade' ? `<b>Trade valuation estimate</b><span>Buyer-side fee (${buyerRate * 100}%): ${money(buyerValue * buyerRate)}</span><span>Other-side fee (${sellerRate * 100}%): ${money(sellerValue * sellerRate)}</span><strong>Total valuation fees: ${money(buyerValue * buyerRate + sellerValue * sellerRate)}</strong>` : `<b>Purchase estimate</b><span>Payment method: ${safe(paymentMethod)}</span><span>Buyer marketplace fee (${buyerRate * 100}%): ${money(buyerValue * buyerRate)}</span><span>Seller marketplace fee (${sellerRate * 100}%): ${money(buyerValue * sellerRate)}</span><span>Delivery: ${miles.toFixed(1)} mi · ${money(courier)} (${money(Math.max(8, miles * .20))} route minimum/rate + ${money(packaging)} packaging)</span><span>Buyer tax: ${money(tax)} · Buyer shipping/courier: ${money(courier)}</span><strong>Buyer due: ${money(buyerValue + buyerValue * buyerRate + tax + courier)}</strong><strong>Seller fee: ${money(buyerValue * sellerRate)}</strong>`;
}
...
document.addEventListener('submit', async event => { if (event.target.closest('#modal')) { event.preventDefault(); const form = event.target; const showError = error => { modalContent.insertAdjacentHTML('beforeend', `<p class="modal-copy">${safe(error.message)}</p>`); };
  try {
    ...
    const trade = form.matches('.fee-calculator[data-type="trade"]'); const purchase = form.matches('.fee-calculator[data-type="purchase"]'); if (purchase) { if (!session) return openAuthPanel('login'); const submit = form.querySelector('.transaction-submit'); submit.disabled = true; submit.textContent = 'Creating order…'; const delivery = await api('/purchase', { method: 'POST', body: JSON.stringify({ listingId: form.dataset.listing, shippingAddress: form.querySelector('#shipping-address').value, deliveryProvider: form.querySelector('#delivery-provider').value, deliveryMiles: form.querySelector('#delivery-miles').value, packagingCost: form.querySelector('#packaging').value, paymentMethod: form.querySelector('[name="paymentMethod"]:checked')?.value }) }); await loadDeliveries(); await loadMarket(); await openDelivery(delivery.id); return; } modalContent.innerHTML = trade ? '<h2 class="modal-title">Trade offer sent.</h2><p class="modal-copy">Your offer is pending with the collector. A trade chat is now open for delivery details and follow-up.</p>' : '<h2 class="modal-title">Saved.</h2><p class="modal-copy">Your update was saved.</p>';
...
document.addEventListener('input', event => { if (event.target.closest('.fee-calculator')) updateFeeSummary(); if (event.target.closest('.trade-builder-form')) updateTradeSelectionCounts(); if (event.target.matches('[name="imageUrls"]')) syncListingPreview(); });
document.addEventListener('change', event => { if (event.target.closest('.trade-builder-form')) updateTradeSelectionCounts(); });
