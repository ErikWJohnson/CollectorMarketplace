...
let listings = [], auctions = [], accounts = [], collectives = [], brands = [], couriers = [], deliveries = [], uploadedListingImages = [], uploadedListingVideos = [], activeCategory = 'All', activeQuery = '', activeTags = [], searchScope = 'listings', sortMode = 'recent', page = 1, observer, searchRenderTimer, auctionClock;
const searchScopes = ['listings', 'accounts', 'collectives', 'brands', 'couriers'];
const setSearchScope = (scope, syncRoute = true) => {
  ...
  search.placeholder = searchScope === 'listings' ? 'Search collector finds' : searchScope === 'accounts' ? 'Search collector accounts' : searchScope === 'collectives' ? 'Search collector collectives' : searchScope === 'brands' ? 'Search collectible brands' : 'Search delivery couriers';
  renderFeed();
...
const directoryItemTags = (item, type) => type === 'accounts' ? [...new Set(listings.filter(listing => listing.ownerId === item.id).flatMap(listingTags).map(tag => tag.toLowerCase()))] : (item.tags || []).map(tag => String(tag).toLowerCase());
...
const filteredDirectory = () => {
  const source = searchScope === 'accounts' ? accounts : searchScope === 'brands' ? brands : searchScope === 'couriers' ? couriers : collectives;
...
const routeTags = () => {
  ...
  const knownTags = [...new Set([...categoryOptions, ...Object.values(tagGroups).flat(), ...listings.flatMap(listingTags), ...couriers.flatMap(courier => courier.tags || [])])];
