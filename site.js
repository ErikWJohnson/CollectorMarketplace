...
let listings = [], auctions = [], accounts = [], collectives = [], deliveries = [], uploadedListingImages = [], uploadedListingVideos = [], activeCategory = 'All', activeQuery = '', activeTags = [], searchScope = 'listings', sortMode = 'recent', page = 1, observer, searchRenderTimer, auctionClock;
...
const directoryCard = (item, type) => type === 'accounts' ? `<article class="search-directory-card" data-profile="${safe(item.id)}"><b>@${safe(item.username)}</b><span>${safe(item.bio || 'Collector profile')}</span><small>${Number(item.activeListingCount || 0)} active listings · ${Number(item.reputation || 0)} reputation</small></article>` : `<article class="search-directory-card collective-card"><b>${safe(item.name)}</b><span>${safe(item.description)}</span><small>${Number(item.members || 0).toLocaleString()} members · ${(item.tags || []).map(tag => '#' + safe(tag)).join(' ')}</small></article>`;
function renderFeed(reset = true) { if (searchScope !== 'listings') { const source = searchScope === 'accounts' ? accounts : collectives; const query = activeQuery.toLowerCase(); const rows = source.filter(item => `${item.username || ''} ${item.name || ''} ${item.bio || ''} ${item.description || ''} ${(item.tags || []).join(' ')}`.toLowerCase().includes(query)); stream.innerHTML = rows.map(item => directoryCard(item, searchScope)).join('') || `<p class="load-state">No ${searchScope} match that search.</p>`; document.querySelector('#result-count').textContent = `${rows.length} ${searchScope}`; sentinel.textContent = 'Directory search complete.'; return; } const rows = filtered(); if (reset) page = 1; const visible = rows.slice(0, page * 4); stream.innerHTML = visible.map(card).join('') || '<p class="load-state">No collector finds match that search.</p>'; stream.querySelectorAll('.collector-head').forEach((header, index) => { header.dataset.profile = visible[index]?.ownerId || ''; header.tabIndex = 0; header.setAttribute('role', 'button'); header.setAttribute('aria-label', `Open ${visible[index]?.owner?.username || 'collector'} profile`); }); document.querySelector('#result-count').textContent = `${rows.length} listed`; sentinel.textContent = page * 4 < rows.length ? 'Scroll for more finds ↓' : 'You are all caught up.'; }
...
document.addEventListener('click', event => {
  ...
  const scopeButton = event.target.closest('[data-search-scope]'); if (scopeButton) { searchScope = scopeButton.dataset.searchScope; document.querySelectorAll('[data-search-scope]').forEach(button => button.classList.toggle('active', button === scopeButton)); search.placeholder = searchScope === 'listings' ? 'Search collector finds' : searchScope === 'accounts' ? 'Search collector accounts' : 'Search collector collectives'; renderFeed(); return; }
