...
const managedListingCard = item => { const archived = item.status === 'archived'; const controls = archived ? `<button type="button" data-listing-edit="${safe(item.id)}">Edit</button><button type="button" data-listing-archive="${safe(item.id)}" data-listing-next-status="active">Restore</button><button type="button" class="listing-delete" data-listing-delete="${safe(item.id)}">Delete</button>` : `<button type="button" data-listing-archive="${safe(item.id)}" data-listing-next-status="archived" data-listing-edit-after="true">Archive & edit</button><button type="button" class="listing-delete" data-listing-delete="${safe(item.id)}">Delete</button>`; return `<article class="managed-listing-card ${archived ? 'is-archived' : ''}"><img src="${safe(item.images?.[0] || item.image || '')}" alt="${safe(item.title)}"><div><small>${archived ? 'ARCHIVED · hidden from marketplace' : 'ACTIVE · visible in marketplace'}</small><b>${safe(item.title)}</b><span>${money(item.price)} · ${safe(item.condition || 'Condition not specified')}</span></div><footer>${controls}</footer></article>`; };
