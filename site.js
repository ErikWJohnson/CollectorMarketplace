...
const collectorFeeRate = user => user?.developerPass === true ? 0 : user?.curator === true || user?.membership === 'curator' ? .01 : .04;
...
document.addEventListener('click', event => {
  ...
  const listingArchive = event.target.closest('[data-listing-archive]'); if (listingArchive) { api(`/listing/${listingArchive.dataset.listingArchive}`, { method: 'PUT', body: JSON.stringify({ status: listingArchive.dataset.listingNextStatus }) }).then(async () => { await loadMarket(); await openAccountPanel(); }).catch(showError); return; }
...
document.addEventListener('input', event => { if (event.target.closest('.listing-form')) saveListingDraft(event.target.closest('.listing-form')); if (event.target.closest('.fee-calculator')) updateFeeSummary(); if (event.target.closest('.trade-builder-form')) { if (event.target.matches('[name="senderCashAmount"], [name="receiverCashAmount"]')) syncTradeCashInput(event.target); updateTradeSelectionCounts(); } if (event.target.matches('[name="imageUrls"]')) syncListingPreview(); });
