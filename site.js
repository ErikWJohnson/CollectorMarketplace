...
stream.addEventListener('wheel', event => { if (browseMode !== 'conveyor') return; const description = event.target.closest('.listing-description'); if (description && description.scrollHeight > description.clientHeight) { const delta = event.deltaY; const canScrollUp = delta < 0 && description.scrollTop > 0; const canScrollDown = delta > 0 && description.scrollTop + description.clientHeight < description.scrollHeight - 1; if (canScrollUp || canScrollDown) return; } event.preventDefault(); stream.scrollLeft += event.deltaY || event.deltaX; }, { passive: false });
