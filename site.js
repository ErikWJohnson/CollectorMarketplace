...
const card = item => `<article class="listing"><aside class="rail"><b>AC</b></aside><div class="listing-main"><header class="seller"><div><strong>@alexcollects</strong><small>★★★★★ · 98 reputation</small></div><time>new drop</time></header><button class="media" data-detail="${item.id}"><img src="${safe(item.image)}" alt="${safe(item.title)}"><span class="grade">${safe(item.category)}</span></button><section class="listing-copy"><h2>${safe(item.title)}</h2><div class="meta">${safe(item.category)} · authentic collector listing</div><p>${safe(item.description)}</p><footer><span class="price">$${item.price}</span><button class="trade" data-trade="${item.id}">${item.trade ? 'BUY · TRADE' : 'BUY NOW'}</button><button class="like" aria-label="Like ${safe(item.title)}">♡</button></footer></section></div></article>`;
...
function setQuery(query) { activeQuery = query; search.value = query; document.querySelector('#clear-tags').hidden = !query; renderFeed(); }
...
document.addEventListener('click', event => {
  ...
  if (event.target.matches('[data-sell]')) openModal('List an item', 'Create your collector profile to publish listings and accept trade offers.', '<form class="modal-form"><input required placeholder="Your email"><button>Join the marketplace</button></form>');
  if (event.target.matches('[data-chat]')) openModal('Collector chat', 'Your conversations and trade updates will appear here once you join the marketplace.');
  if (event.target.matches('[data-market]') || event.target.matches('[data-home]')) { activeCategory = 'All'; setQuery(''); renderCategories(); renderFeed(); window.scrollTo({ top: 0, behavior: 'smooth' }); }
...
fetch('data/listings.json').then(response => response.json()).then(data => { listings = data; renderCategories(); renderFeed(); observer = new IntersectionObserver(entries => { if (entries[0].isIntersecting && page * 4 < filtered().length) { page++; renderFeed(false); } }, { rootMargin: '250px' }); observer.observe(sentinel); }).catch(() => { stream.innerHTML = '<p class="load-state">The marketplace feed could not load. Please refresh the page.</p>'; });
