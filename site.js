...
async function renderPayPalButtonForPurchase(form) {
  ...
  try {
    const buttons = paypal.Buttons({
      createOrder: async () => { if (!form.reportValidity()) throw new Error('Complete the required delivery details first.'); order = await api('/paypal/orders', { method: 'POST', body: JSON.stringify({ listingId: form.dataset.listing, shippingAddress: form.querySelector('#shipping-address').value, deliveryProvider: form.querySelector('#delivery-provider').value, deliveryMiles: form.querySelector('#delivery-miles').value, paymentMethod: form.querySelector('[name="paymentMethod"]:checked')?.value }) }); return order.orderId; },
      onApprove: async () => { try { const delivery = await api(`/paypal/orders/${order.deliveryId}/capture`, { method: 'POST', body: '{}' }); await loadDeliveries(); await loadMarket(); await openDelivery(delivery.id); } catch (error) { container.innerHTML = `<p class="listing-submit-error" role="alert">${safe(error.message)}</p>`; } },
      onCancel: () => order ? api(`/paypal/orders/${order.deliveryId}/cancel`, { method: 'POST', body: '{}' }).then(() => { order = null; }).catch(showError) : undefined,
      onError: () => { container.insertAdjacentHTML('beforeend', '<p class="listing-submit-error" role="alert">PayPal could not continue. No payment was captured.</p>'); }
    });
