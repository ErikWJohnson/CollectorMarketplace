...
function updateFeeSummary() {
  ...
  const paymentMethod = document.querySelector('[name="paymentMethod"]:checked')?.value || 'Select a payment method'; const deliveryFee = document.querySelector('#delivery-fee'); if (deliveryFee) deliveryFee.textContent = `Calculated delivery fee · ${money(courier)} (${money(Math.max(8, miles * .20))} route minimum/rate + ${money(packaging)} packaging)`;
