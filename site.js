...
function feeCalculator(item, type) {
  ...
  const savedZip = safe(zipFromAddress(checkoutPreferences.address));
  const address = trade ? '' : `<section class="shipping-address-fields"><h3>Delivery address</h3><p>Used privately for the real UPS label after payment. It is shared only with the seller.</p><label>Recipient name<input required id="shipping-name" autocomplete="shipping name" maxlength="120" placeholder="Full name"></label><label>Street address<input required id="shipping-street1" autocomplete="shipping street-address" maxlength="160" placeholder="Street and number"></label><label>Address line 2 <i>Optional</i><input id="shipping-street2" autocomplete="shipping address-line2" maxlength="120" placeholder="Apartment, suite, etc."></label><div><label>City<input required id="shipping-city" autocomplete="shipping address-level2" maxlength="80"></label><label>State<input required id="shipping-state" autocomplete="shipping address-level1" maxlength="2" pattern="[A-Za-z]{2}" placeholder="CA"></label><label>ZIP code<input required id="shipping-zip" autocomplete="shipping postal-code" inputmode="numeric" pattern="[0-9]{5}(-[0-9]{4})?" maxlength="10" value="${savedZip}" placeholder="94103"></label></div></section>`;
...
document.addEventListener('submit', async event => { const form = event.target; const appSection = form.closest('.app-section-page'); if (form.closest('#modal') || appSection) { event.preventDefault(); const showError = error => { const host = appSection?.querySelector('.app-section-content') || modalContent; host.querySelector('.form-submit-error')?.remove(); host.insertAdjacentHTML('afterbegin', `<p class="modal-copy form-submit-error" role="alert">${safe(error.message)}</p>`); };
  try {
    ...
    const commentForm = form.closest('.comment-form'); if (commentForm) { const item = listings.find(row => row.id === commentForm.dataset.post); const comment = commentForm.querySelector('textarea').value.trim(); const mediaUrl = String(new FormData(commentForm).get('mediaUrl') || '').trim(); if (!session) return openAuthPanel('login'); if (comment) { await api('/comment', { method: 'POST', body: JSON.stringify({ listingId: item.id, body: comment, mediaUrl, parentId: commentForm.dataset.parent || undefined }) }); await loadMarket(); const refreshed = listings.find(row => row.id === item.id) || item; await openComments(refreshed); } return; }
