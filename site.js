...
async function openDelivery(id) {
  ...
  const deliverySummary = `${safe(provider)} · ${Number(delivery.deliveryMiles || 0).toFixed(1)} mi · ${money(delivery.courierPay || 0)} delivery estimate${delivery.trackingNumber ? ` · Tracking ${safe(delivery.trackingNumber)}` : trackingRequired ? ' · Tracking pending' : ' · Handoff confirmation required'}`;
...
document.addEventListener('submit', async event => { if (event.target.closest('#modal')) { event.preventDefault(); const form = event.target; const showError = error => { modalContent.insertAdjacentHTML('beforeend', `<p class="modal-copy">${safe(error.message)}</p>`); };
  try {
    ...
    const trade = form.matches('.fee-calculator[data-type="trade"]'); const purchase = form.matches('.fee-calculator[data-type="purchase"]'); if (purchase) { if (!session) return openAuthPanel('login'); const submit = form.querySelector('.transaction-submit'); submit.disabled = true; submit.textContent = 'Creating order…'; const delivery = await api('/purchase', { method: 'POST', body: JSON.stringify({ listingId: form.dataset.listing, shippingAddress: form.querySelector('#shipping-address').value, deliveryProvider: form.querySelector('#delivery-provider').value, deliveryMiles: form.querySelector('#delivery-miles').value, packagingCost: form.querySelector('#packaging').value }) }); await loadDeliveries(); await loadMarket(); await openDelivery(delivery.id); return; } modalContent.innerHTML = trade ? '<h2 class="modal-title">Trade offer sent.</h2><p class="modal-copy">Your offer is pending with the collector. A trade chat is now open for delivery details and follow-up.</p>' : '<h2 class="modal-title">Saved.</h2><p class="modal-copy">Your update was saved.</p>';
