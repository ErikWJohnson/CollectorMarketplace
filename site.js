...
function updateFeeSummary() {
  ...
  document.querySelector('#fee-summary').innerHTML = type === 'trade' ? `<b>Trade valuation estimate</b><span>Buyer-side fee (${buyerRate * 100}%): ${money(buyerValue * buyerRate)}</span><span>Other-side fee (${sellerRate * 100}%): ${money(sellerValue * sellerRate)}</span><strong>Total valuation fees: ${money(buyerValue * buyerRate + sellerValue * sellerRate)}</strong>` : `<b>Purchase estimate</b><span>Payment method: ${safe(paymentMethod)}</span><span>Buyer marketplace fee (${buyerRate * 100}%): ${money(buyerMarketplaceFee)}</span><span>Seller marketplace fee (${sellerRate * 100}%): ${money(buyerValue * sellerRate)}</span><span>Delivery: ${miles.toFixed(1)} mi · ${money(courier)} (${money(Math.max(8, miles * .20))} route minimum/rate + ${money(packaging)} packaging)</span><span>Buyer tax: ${money(tax)} · Buyer shipping/courier: ${money(courier)}</span>${minimumBuyerFee ? `<span>Minimum buyer fee (items under $10): ${money(minimumBuyerFee)}</span>` : ''}<span>PayPal processing (3.49% + $0.49): ${money(paypalFee)}</span><strong>Buyer due: ${money(buyerSubtotal + minimumBuyerFee + paypalFee)}</strong><strong>Seller fee: ${money(buyerValue * sellerRate)}</strong>`;
}
