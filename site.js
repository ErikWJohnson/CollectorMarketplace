...
let checkoutPreferences = { address: '', paymentMethod: 'PayPal', deliveryProvider: 'UPS Priority', coordinates: null };
...
const renderCheckoutPreferenceButtons = () => { const location = document.querySelector('[data-checkout-location]'); const payment = document.querySelector('[data-checkout-payment]'); const delivery = document.querySelector('[data-checkout-delivery]'); if (location) location.textContent = checkoutPreferences.address ? '⌖ ZIP code set' : checkoutPreferences.coordinates ? '⌖ Current location set' : '⌖ ZIP code'; if (payment) payment.textContent = `Payment · ${checkoutPreferences.paymentMethod || 'PayPal'}`; if (delivery) delivery.textContent = `Delivery · ${checkoutPreferences.deliveryProvider || 'UPS Priority'}`; };
...
async function openPayPalSandboxCheckout(order) {
  const paypal = await ensurePayPalSdk();
  openModal('PayPal Sandbox checkout', 'Approve this test payment securely with PayPal. No real funds move in Sandbox.', '<div id="paypal-button-container" class="paypal-button-container"></div><p class="policy-note">Canceling returns this item to the marketplace.</p>');
  try {
    const buttons = paypal.Buttons({
      createOrder: () => order.orderId,
      onApprove: async () => { try { const delivery = await api(`/paypal/orders/${order.deliveryId}/capture`, { method: 'POST', body: '{}' }); await loadDeliveries(); await loadMarket(); await openDelivery(delivery.id); } catch (error) { modalContent.innerHTML = `<h2 class="modal-title">Payment needs attention</h2><p class="modal-copy">${safe(error.message)}</p>`; } },
      onCancel: () => api(`/paypal/orders/${order.deliveryId}/cancel`, { method: 'POST', body: '{}' }).then(() => openModal('PayPal Sandbox checkout cancelled', 'No Sandbox payment was captured, and the listing is available again.')).catch(showError),
      onError: () => { modalContent.innerHTML = '<h2 class="modal-title">PayPal checkout could not continue</h2><p class="modal-copy">No payment was captured. Close this window and try again after disabling any PayPal or content-blocking extension.</p>'; }
    });
    if (!buttons.isEligible()) throw new Error('PayPal checkout is not available in this browser.');
    await buttons.render('#paypal-button-container');
  } catch (error) { await api(`/paypal/orders/${order.deliveryId}/cancel`, { method: 'POST', body: '{}' }).catch(() => {}); throw error; }
}
...
const managedListingCard = item => { const archived = item.status === 'archived'; const controls = archived ? `<button type="button" data-listing-edit="${safe(item.id)}">Edit</button><button type="button" data-listing-archive="${safe(item.id)}" data-listing-next-status="active">Restore</button><button type="button" class="listing-delete" data-listing-delete="${safe(item.id)}">Delete</button>` : `<button type="button" data-listing-archive="${safe(item.id)}" data-listing-next-status="archived" data-listing-edit-after="true">Save for later & edit</button><button type="button" class="listing-delete" data-listing-delete="${safe(item.id)}">Delete</button>`; return `<article class="managed-listing-card ${archived ? 'is-archived' : ''}"><img src="${safe(item.images?.[0] || item.image || '')}" alt="${safe(item.title)}"><div><small>${archived ? 'ARCHIVED · hidden from marketplace' : 'ACTIVE · visible in marketplace'}</small><b>${safe(item.title)}</b><span>${money(item.price)} · ${safe(item.condition || 'Condition not specified')}</span></div><footer>${controls}</footer></article>`; };
...
openListingForm = () => { modal.style.display = 'none'; openListingModal(); const markup = modalContent.innerHTML; restoringWorkspaceHistory = true; modal.close(); openAppSection('listing', 'Create a listing', 'Build a complete collector-ready listing with media, delivery details, and a saved draft.', markup); document.body.classList.add('listing-page'); requestAnimationFrame(() => { modal.style.display = ''; restoringWorkspaceHistory = false; }); };
...
async function openDelivery(id) {
  ...
  const shippo = delivery.shippo || {}; const shippoControls = seller && !shippo.transactionId ? `<form class="modal-form shippo-rate-form" data-delivery="${delivery.id}"><h3>UPS label</h3><p>Request a live UPS quote first. Buying the label is a separate, chargeable step.</p><label>Sender name<input required name="originName" value="${safe(shippo.origin?.name || '')}"></label><label>Sender street<input required name="originStreet1" value="${safe(shippo.origin?.street1 || '')}"></label><label>Sender city / state / ZIP<input required name="originCity" placeholder="City" value="${safe(shippo.origin?.city || '')}"><input required name="originState" placeholder="State" value="${safe(shippo.origin?.state || '')}"><input required name="originZip" placeholder="ZIP" value="${safe(shippo.origin?.zip || '')}"></label><label>Recipient name<input required name="destinationName" value="${safe(shippo.destination?.name || '')}"></label><label>Recipient street<input required name="destinationStreet1" value="${safe(shippo.destination?.street1 || '')}"></label><label>Recipient city / state / ZIP<input required name="destinationCity" placeholder="City" value="${safe(shippo.destination?.city || '')}"><input required name="destinationState" placeholder="State" value="${safe(shippo.destination?.state || '')}"><input required name="destinationZip" placeholder="ZIP" value="${safe(shippo.destination?.zip || '')}"></label><label>Package L × W × H (in)<input required name="length" type="number" min=".1" step=".1" value="${safe(shippo.parcel?.length || '')}"><input required name="width" type="number" min=".1" step=".1" value="${safe(shippo.parcel?.width || '')}"><input required name="height" type="number" min=".1" step=".1" value="${safe(shippo.parcel?.height || '')}"></label><label>Package weight (lb)<input required name="weight" type="number" min=".1" step=".1" value="${safe(shippo.parcel?.weight || '')}"></label><button type="submit">${shippo.rateId ? `Refresh UPS quote · ${money(shippo.amount)}` : 'Get UPS rate'}</button></form>${shippo.rateId ? `<button type="button" class="auth-switch" data-shippo-buy="${delivery.id}">Buy UPS label · ${money(shippo.amount)}</button>` : ''}` : shippo.transactionId ? `<section class="delivery-fee-summary"><h3>Shippo label</h3><span>${safe(shippo.provider)} · ${safe(shippo.service)}</span><span>Tracking · ${safe(delivery.trackingNumber || 'pending')}</span>${shippo.labelUrl ? `<a href="${safe(shippo.labelUrl)}" target="_blank" rel="noopener">Open shipping label</a>` : ''}</section>` : '';
...
function feeCalculator(item, type) {
  ...
  const action = trade ? 'Send trade offer & open chat' : 'Continue to PayPal Sandbox';
  const address = trade ? '' : `<label>Delivery address<textarea required id="shipping-address" maxlength="500" autocomplete="street-address" placeholder="Street, city, state, ZIP">${safe(checkoutPreferences.address)}</textarea></label>`;
  ...
  return `<form class="fee-calculator purchase-checkout" data-type="${type}" data-listing="${item.id}" data-seller="${safe(item.ownerId || '')}" data-seller-zip="${safe(item.sellerZip || '')}"><section class="purchase-fields"><header><span>1</span><div><b>Delivery details</b><small>Choose a provider, estimate the route, and confirm where this collector item should go.</small></div></header>${fields}${address}${paymentChoices}</section><aside class="purchase-review"><header><span>2</span><div><b>Order review</b><small>Review the item, fees, delivery estimate, and payment choice.</small></div></header>${itemSummary}<div id="fee-summary" class="fee-summary" aria-live="polite"></div>${agreement}<p class="policy-note">You will be sent to PayPal Sandbox to approve this test checkout. No real payment is collected. After Sandbox approval, this listing is reserved and a private delivery thread opens with the seller. Buyers pay taxes and shipping. Courier delivery pay is the greater of $8 or $0.10 per mile, plus documented packaging costs.</p><button type="submit" class="transaction-submit">${action}</button></aside></form>`;
}
...
document.addEventListener('click', event => {
  ...
  if (event.target.closest('[data-checkout-delivery]')) { openModal('Delivery option', 'Choose your default delivery option for checkout.', '<form class="modal-form checkout-preferences-form" data-preference="delivery"><fieldset class="payment-methods"><label class="payment-method"><input type="radio" name="deliveryProvider" value="UPS Priority" checked><span><b>UPS Priority</b><small>Tracked carrier delivery</small></span></label></fieldset><button>Save delivery option</button></form>'); return; }
...
document.addEventListener('submit', async event => { const form = event.target; const appSection = form.closest('.app-section-page'); if (form.closest('#modal') || appSection) { event.preventDefault(); const showError = error => { const host = appSection?.querySelector('.app-section-content') || modalContent; host.querySelector('.form-submit-error')?.remove(); host.insertAdjacentHTML('afterbegin', `<p class="modal-copy form-submit-error" role="alert">${safe(error.message)}</p>`); };
  try {
    const checkoutPreferencesForm = form.closest('.checkout-preferences-form'); if (checkoutPreferencesForm) { const fields = new FormData(checkoutPreferencesForm); if (checkoutPreferencesForm.dataset.preference === 'location') { checkoutPreferences.address = String(fields.get('address') || '').trim(); await refreshCheckoutEstimates(); } else if (checkoutPreferencesForm.dataset.preference === 'delivery') checkoutPreferences.deliveryProvider = String(fields.get('deliveryProvider') || 'UPS Priority'); else checkoutPreferences.paymentMethod = String(fields.get('paymentMethod') || 'PayPal'); saveCheckoutPreferences(); renderCheckoutPreferenceButtons(); modal.close(); return; }
    ...
    const trade = form.matches('.fee-calculator[data-type="trade"]'); const purchase = form.matches('.fee-calculator[data-type="purchase"]'); if (purchase) { if (!session) return openAuthPanel('login'); const submit = form.querySelector('.transaction-submit'); submit.disabled = true; submit.textContent = 'Loading secure checkout…'; await ensurePayPalSdk(); const order = await api('/paypal/orders', { method: 'POST', body: JSON.stringify({ listingId: form.dataset.listing, shippingAddress: form.querySelector('#shipping-address').value, deliveryProvider: form.querySelector('#delivery-provider').value, deliveryMiles: form.querySelector('#delivery-miles').value, paymentMethod: form.querySelector('[name="paymentMethod"]:checked')?.value }) }); await openPayPalSandboxCheckout(order); return; } modalContent.innerHTML = trade ? '<h2 class="modal-title">Trade offer sent.</h2><p class="modal-copy">Your offer is pending with the collector. A trade chat is now open for delivery details and follow-up.</p>' : '<h2 class="modal-title">Saved.</h2><p class="modal-copy">Your update was saved.</p>';
...
document.addEventListener('input', event => { if (event.target.closest('.listing-form')) saveListingDraft(event.target.closest('.listing-form')); if (event.target.closest('.fee-calculator')) updateFeeSummary(); if (event.target.matches('#shipping-address')) { clearTimeout(checkoutAddressTimer); checkoutAddressTimer = setTimeout(() => calculateCheckoutDeliveryFromAddress(event.target), 500); } if (event.target.closest('.trade-builder-form')) { if (event.target.matches('[name="senderCashAmount"], [name="receiverCashAmount"]')) syncTradeCashInput(event.target); updateTradeSelectionCounts(); } if (event.target.matches('[name="imageUrls"]')) syncListingPreview(); });
