...
document.addEventListener('click', event => {
  ...
  if (event.target.closest('[data-checkout-location]')) { openModal('Delivery address', 'Save an address with a US ZIP code to show estimated buy totals across listings. It is kept only in this browser.', `<form class="modal-form checkout-preferences-form" data-preference="location"><textarea required name="address" maxlength="500" autocomplete="street-address" placeholder="Street, city, state, ZIP">${safe(checkoutPreferences.address)}</textarea><button>Save delivery address</button></form>`); return; }
