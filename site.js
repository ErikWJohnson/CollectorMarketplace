...
const tradeItemRows = (items, name, checked = false, required = false) => items.map(item => `<label class="trade-item-option"><input type="checkbox" name="${name}" value="${safe(item.id)}" ${checked ? 'checked' : ''} ${required ? 'required' : ''}><img src="${safe(item.image || item.images?.[0] || '')}" alt=""><span><b>${safe(item.title)}</b><small>${money(item.price)}</small></span></label>`).join('');
const tradeSummary = trade => { const senderItems = trade.senderListings || []; const receiverItems = trade.receiverListings || (trade.listing ? [trade.listing] : []); const sentByYou = trade.senderId === session?.user?.id; const theyGive = sentByYou ? receiverItems : senderItems; const youGive = sentByYou ? senderItems : receiverItems; const cash = Number(trade.cashAmount || 0); const youAddCash = (trade.cashFrom === 'sender') === sentByYou; const cashLine = cash ? `<p class="trade-cash-adjustment">${youAddCash ? 'You' : safe(trade.otherUser?.username || 'Other collector')} add${youAddCash ? '' : 's'} ${money(cash)} cash</p>` : ''; return `<section class="trade-item-summary"><div><h3>They give</h3>${theyGive.map(item => `<span>${safe(item.title)} · ${money(item.price)}</span>`).join('')}</div><div><h3>You give</h3>${youGive.map(item => `<span>${safe(item.title)} · ${money(item.price)}</span>`).join('')}</div>${cashLine}</section>`; };
async function openTradeChat(id) { const trade = await api(`/trade/${id}`); const messages = trade.messages.length ? trade.messages.map(message => `<p class="post-comment ${message.senderId === session.user.id ? 'is-own-message' : ''}"><b>@${safe(message.senderId === session.user.id ? session.user.username : trade.otherUser?.username || 'collector')}</b><span>${safe(message.body)}</span></p>`).join('') : '<p class="modal-copy">No messages yet. Discuss the trade here.</p>'; const ownerControls = trade.receiverId === session.user.id && trade.status === 'pending' ? `<button type="button" data-trade-status="accepted" data-trade-id="${trade.id}">Accept trade</button><button type="button" data-trade-status="declined" data-trade-id="${trade.id}">Decline trade</button>` : ''; const completeControl = trade.status === 'accepted' ? `<button type="button" data-trade-status="completed" data-trade-id="${trade.id}">Mark trade completed</button>` : ''; const title = trade.receiverListings?.map(item => item.title).join(' + ') || trade.listing?.title || 'Collector trade'; openModal(`Trade · ${title}`, `Status: ${safe(trade.status)}`, `<section class="conversation-workspace trade-conversation-workspace"><header class="conversation-header"><button type="button" data-chat>← All messages</button><span class="trade-status">${safe(trade.status.replaceAll('_', ' '))}</span><strong>${safe(title)}</strong><button type="button" class="voice-start" data-voice-room="trade:${trade.id}" data-voice-label="Trade voice · ${safe(title)}">Start voice</button></header>${tradeSummary(trade)}<section class="delivery-thread collector-thread trade-thread" aria-live="polite">${messages}</section><footer class="trade-actions">${ownerControls}${completeControl || '<span>Use this thread to coordinate the exchange.</span>'}</footer><form class="modal-form trade-message-form collector-message-form" data-trade="${trade.id}"><textarea required name="body" maxlength="500" placeholder="Message the other collector"></textarea><button type="submit">Send message</button></form></section>`); modal.classList.add('chat-dialog', 'trade-dialog'); document.body.classList.add('chat-open'); requestAnimationFrame(() => { const thread = modal.querySelector('.trade-thread'); if (thread) thread.scrollTop = thread.scrollHeight; modal.querySelector('.trade-message-form textarea')?.focus(); }); }
function openTradeOfferChat(item) { const myItems = listings.filter(row => row.ownerId === session?.user?.id && row.status === 'active'); const theirItems = listings.filter(row => row.ownerId === item.ownerId && row.status === 'active'); openModal(`New trade · ${item.title}`, 'Choose one or more items from each collector. You can also add a cash adjustment to balance the deal.', `<form class="modal-form trade-builder-form" data-listing="${item.id}" data-receiver="${item.ownerId}"><section class="trade-builder-columns"><section><h3>You give</h3><p>Select one or more of your active listings.</p><div class="trade-item-list">${myItems.length ? tradeItemRows(myItems, 'senderListingIds') : '<p class="modal-copy">You need an active listing before you can offer a trade.</p>'}</div></section><section><h3>@${safe(item.owner?.username || item.ownerName || 'collector')} gives</h3><p>The selected listing is already included; add more if you want.</p><div class="trade-item-list">${tradeItemRows(theirItems, 'receiverListingIds', false)}</div></section></section><section class="trade-cash-fields"><label>Cash adjustment<input name="cashAmount" type="number" min="0" max="1000000" step="0.01" value="0"></label><label>Who adds cash?<select name="cashFrom"><option value="">No cash adjustment</option><option value="sender">I add cash</option><option value="receiver">They add cash</option></select></label></section><label>Offer note <i>Optional</i><textarea name="offerDetails" maxlength="1000" placeholder="Explain the condition, extras, or proposed exchange details."></textarea></label><button type="submit" class="transaction-submit">Send trade offer &amp; open chat</button></form>`); modal.classList.add('chat-dialog', 'trade-dialog'); document.body.classList.add('chat-open'); const target = modal.querySelector(`[name="receiverListingIds"][value="${CSS.escape(item.id)}"]`); if (target) { target.checked = true; target.closest('.trade-item-option')?.classList.add('is-required'); } }
...
document.addEventListener('submit', async event => { if (event.target.closest('#modal')) { event.preventDefault(); const form = event.target; const showError = error => { modalContent.insertAdjacentHTML('beforeend', `<p class="modal-copy">${safe(error.message)}</p>`); };
  try {
    ...
    const tradeBuilder = form.matches('.trade-builder-form'); if (tradeBuilder) { if (!session) return openAuthPanel('login'); const fields = new FormData(form); const selected = name => [...fields.getAll(name)].filter(Boolean); const senderListingIds = selected('senderListingIds'); const receiverListingIds = [...new Set([...selected('receiverListingIds'), form.dataset.listing])]; const cashAmount = Number(fields.get('cashAmount') || 0); const cashFrom = String(fields.get('cashFrom') || ''); if (!senderListingIds.length) throw new Error('Choose at least one of your listings to offer.'); if (!receiverListingIds.length) throw new Error('Choose at least one listing to receive.'); if (!Number.isFinite(cashAmount) || cashAmount < 0) throw new Error('Use a valid cash adjustment.'); if (cashAmount && !cashFrom) throw new Error('Choose which collector adds the cash adjustment.'); const trade = await api('/trade', { method: 'POST', body: JSON.stringify({ listingId: form.dataset.listing, receiverId: form.dataset.receiver, senderListingIds, receiverListingIds, cashAmount, cashFrom, offerDetails: String(fields.get('offerDetails') || '') }) }); await openTradeChat(trade.id); return; }
