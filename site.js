...
const renderCheckoutPreferenceButtons = () => { const location = document.querySelector('[data-checkout-location]'); const payment = document.querySelector('[data-checkout-payment]'); if (location) location.textContent = checkoutPreferences.address ? '⌖ ZIP code set' : checkoutPreferences.coordinates ? '⌖ Current location set' : '⌖ ZIP code'; if (payment) payment.textContent = `Payment · ${checkoutPreferences.paymentMethod || 'PayPal'}`; };
