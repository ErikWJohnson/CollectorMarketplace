...
function openAppSection(kind, title, copy, content = '') {
  ...
  stream.innerHTML = `<section class="app-section-page"><header class="app-section-intro"><span>${kind === 'account' ? 'Collector workspace' : 'Social workspace'}</span><h1>${title}</h1><p>${copy || ''}</p></header><div class="app-section-content">${content}</div></section>`;
...
function openModal(title, copy, form) {
  ...
  if (form?.includes('listing-form')) {
    const primaryTag = modalContent.querySelector('[name="category"]');
    const extraTags = modalContent.querySelector('[name="tags"]');
    primaryTag?.closest('label')?.firstChild && (primaryTag.closest('label').firstChild.textContent = 'Tags · start with one primary tag');
    if (primaryTag) primaryTag.placeholder = 'Add your first tag';
    extraTags?.closest('label')?.firstChild && (extraTags.closest('label').firstChild.textContent = 'More tags ');
    modalContent.querySelector('.listing-form-grid')?.insertAdjacentHTML('afterend', `<section class="listing-location-panel"><p class="listing-step">02 · LOCATION & HANDOFF</p><div class="listing-location-grid"><label>Seller city + state/region<input required name="sellerCity" maxlength="80" autocomplete="address-level2" placeholder="e.g. Portland, OR"></label><label>ZIP code<input required name="sellerZip" inputmode="numeric" autocomplete="postal-code" pattern="[0-9]{5}(-[0-9]{4})?" placeholder="97205"></label><label>Pickup radius (miles)<input required name="pickupRadiusMiles" type="number" min="0" max="500" step="1" value="25"></label></div><fieldset class="listing-fulfillment-field"><legend>Fulfillment</legend><div><label><input required type="radio" name="fulfillment" value="pickup_delivery" checked> Pickup & delivery</label><label><input required type="radio" name="fulfillment" value="pickup"> Pickup only</label></div><small>Your street address stays private. Buyers see only the city and ZIP until an order is confirmed.</small></fieldset><input type="hidden" name="locationLat"><input type="hidden" name="locationLng"><button type="button" class="location-capture" data-capture-location>Use approximate device location for distance</button><span class="location-capture-note">Coordinates are rounded before storage and used only for distance and pickup eligibility.</span></section>`);
    ...
    if (required) required.textContent = 'Required: title, tags, condition, price, city, ZIP, pickup radius, description, and a photo.';
  }
}
...
const tradePaymentOptions = ['Card', 'Apple Pay', 'Google Pay', 'Cash App Pay', 'Venmo', 'PayPal', 'ACH bank transfer'];
...
async function openDelivery(id) {
  ...
  const purchaseFees = delivery.fees ? `<section class="delivery-fee-summary"><h3>Marketplace fees</h3><span>Buyer · ${Number(delivery.fees.buyer.rate * 100)}% · ${money(delivery.fees.buyer.amount)}</span><span>Seller · ${Number(delivery.fees.seller.rate * 100)}% · ${money(delivery.fees.seller.amount)}</span></section>` : '';
...
function feeCalculator(item, type) {
  ...
  const paymentMethods = [['Card', 'Debit or credit card'], ['Apple Pay', 'Apple Wallet'], ['Google Pay', 'Google Wallet'], ['Cash App Pay', 'Cash App'], ['Venmo', 'Venmo balance or linked account'], ['PayPal', 'PayPal account'], ['ACH bank transfer', 'Checking or savings account']];
  ...
  return `<form class="fee-calculator purchase-checkout" data-type="${type}" data-listing="${item.id}" data-seller="${safe(item.ownerId || '')}"><section class="purchase-fields"><header><span>1</span><div><b>Delivery details</b><small>Choose a provider, estimate the route, and confirm where this collector item should go.</small></div></header>${fields}${address}${paymentChoices}</section><aside class="purchase-review"><header><span>2</span><div><b>Order review</b><small>Review the item, fees, delivery estimate, and payment choice.</small></div></header>${itemSummary}<div id="fee-summary" class="fee-summary" aria-live="polite"></div>${agreement}<p class="policy-note">Placing an order reserves this listing and opens a private delivery thread with the seller. Your selected payment method is saved with the order; payment collection remains disabled until production payment providers are configured. Buyers pay taxes and shipping. Courier delivery pay is the greater of $8 or $0.20 per mile, plus documented packaging costs.</p><button type="submit" class="transaction-submit">${action}</button></aside></form>`;
}
function updateFeeSummary() {
  const box = document.querySelector('.fee-calculator'); if (!box) return;
  const type = box.dataset.type; const buyerValue = Number(document.querySelector('#buyer-value')?.value || 0); const sellerValue = Number(document.querySelector('#seller-value')?.value || buyerValue); const seller = accounts.find(account => account.id === box.dataset.seller); const buyerRate = type === 'purchase' ? collectorFeeRate(session?.user) : document.querySelector('#buyer-curator')?.checked ? .01 : .04; const sellerRate = type === 'purchase' ? collectorFeeRate(seller) : document.querySelector('#seller-curator')?.checked ? .01 : .04; const tax = Number(document.querySelector('#tax')?.value || 0); const packaging = Number(document.querySelector('#packaging')?.value || 0); const miles = Number(document.querySelector('#delivery-miles')?.value || 0); const courier = Math.max(8, miles * .20) + packaging;
...
document.addEventListener('submit', async event => { if (event.target.closest('#modal')) { event.preventDefault(); const form = event.target; const showError = error => { modalContent.insertAdjacentHTML('beforeend', `<p class="modal-copy">${safe(error.message)}</p>`); };
  try {
    ...
    const listingForm = form.closest('.listing-form'); if (listingForm) { const fields = new FormData(listingForm); const images = [...uploadedListingImages, ...imageUrls(fields.get('imageUrls') || '')].slice(0, 5); const condition = String(fields.get('condition') || '').trim(); const tags = parseListingTags(fields.get('tags'), fields.get('category'), condition); if (!images.length) throw new Error('Add at least one listing photo or image link.'); if (tags.length > 8) throw new Error('Use up to 8 tags, including the category and condition.'); if (!fields.get('locationLat') || !fields.get('locationLng')) throw new Error('Use approximate device location so buyers can see real distance, ETA, and pickup eligibility.'); const button = listingForm.querySelector('button[type="submit"], button:not([type])'); button.disabled = true; button.textContent = 'Publishing…'; const created = await api('/listing', { method: 'POST', body: JSON.stringify({ title: fields.get('title'), description: fields.get('description'), category: fields.get('category'), condition, tags, sellerCity: fields.get('sellerCity'), sellerZip: fields.get('sellerZip'), pickupRadiusMiles: fields.get('pickupRadiusMiles'), locationCoordinates: { lat: fields.get('locationLat'), lng: fields.get('locationLng') }, fulfillment: fields.get('fulfillment'), listingMode: fields.get('listingMode'), auctionStartPrice: fields.get('auctionStartPrice'), auctionDurationHours: fields.get('auctionDurationHours'), price: fields.get('price'), tradeOffer: fields.get('tradeOffer') === 'on', images, videos: uploadedListingVideos }) }); if (created.listingMode !== 'marketplace') auctions = [...auctions, { ...created, image: created.images?.[0] || '', currentBid: Number(created.auctionStartPrice || 0), bids: 0, endAt: new Date(created.auctionEndAt).valueOf() }]; await loadMarket(); modal.close(); return; }
...
document.addEventListener('input', event => { if (event.target.closest('.fee-calculator')) updateFeeSummary(); if (event.target.closest('.trade-builder-form')) { if (event.target.matches('[name="senderCashAmount"], [name="receiverCashAmount"]')) syncTradeCashInput(event.target); updateTradeSelectionCounts(); } if (event.target.matches('[name="imageUrls"]')) syncListingPreview(); });
