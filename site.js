...
let checkoutPreferences = { address: '', paymentMethod: 'PayPal', coordinates: null };
...
const renderCheckoutPreferenceButtons = () => { const location = document.querySelector('[data-checkout-location]'); const payment = document.querySelector('[data-checkout-payment]'); if (location) location.textContent = checkoutPreferences.address ? '⌖ Delivery address set' : checkoutPreferences.coordinates ? '⌖ Current location set' : '⌖ Delivery address'; if (payment) payment.textContent = `Payment · ${checkoutPreferences.paymentMethod || 'PayPal'}`; };
...
const renderVisibleCheckoutEstimates = () => { if (!checkoutPreferences.coordinates) return; document.querySelectorAll('[data-purchase]').forEach(button => { const item = listings.find(row => row.id === button.dataset.purchase); const seller = item?.locationCoordinates; if (!item || !seller || !Number.isFinite(Number(seller.lat)) || !Number.isFinite(Number(seller.lng))) return; const miles = milesBetween(checkoutPreferences.coordinates, seller); const price = Number(item.price) || 0; const delivery = Math.max(8, miles * .10) + Math.max(0, Number(item.upsPackagingCost) || 0); const subtotal = price + delivery + price * collectorFeeRate(session?.user) + (price < 10 ? 10 : 0); button.innerHTML = `<span>BUY · EST. TOTAL</span><b>${money(subtotal + paypalProcessingFee(subtotal))}</b><small>${miles.toFixed(0)} mi · PayPal · before tax</small>`; }); };
...
document.addEventListener('click', event => {
  ...
  if (event.target.closest('[data-checkout-location]')) { openModal('Delivery address', 'Save an address with a US ZIP code, or use your current approximate location, to show estimated buy totals. This is kept only in this browser.', `<form class="modal-form checkout-preferences-form" data-preference="location"><textarea name="address" maxlength="500" autocomplete="street-address" placeholder="Street, city, state, ZIP">${safe(checkoutPreferences.address)}</textarea><button type="button" class="auth-switch" data-checkout-current-location>Use current location for estimates</button><button>Save delivery address</button></form>`); return; }
