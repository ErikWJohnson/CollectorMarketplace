...
const directoryCard = (item, type) => type === 'accounts' ? `<article class="search-directory-card" data-profile="${safe(item.id)}"><b>@${safe(item.username)}</b><span>${safe(item.bio || 'Collector profile')}</span><small>${Number(item.activeListingCount || 0)} posts · ${Number(item.followingCount || 0)} following · ${Number(item.reputation || 0)} reputation${item.profileTags?.length ? ` · ${(item.profileTags || []).map(tag => '#' + safe(tag)).join(' ')}` : ''}</small></article>` : type === 'brands' ? `<article class="search-directory-card collective-card brand-card"><b>${safe(item.name)}</b><span>${safe(item.description)}</span><small>${safe(item.category || 'Collector brand')}</small></article>` : type === 'couriers' ? `<article class="search-directory-card collective-card courier-card"><b>${safe(item.name)}</b><span>${safe(item.description)}</span><small>${safe(item.category || 'Courier')} · ${(item.tags || []).map(tag => '#' + safe(tag)).join(' ')}</small></article>` : type === 'chatrooms' ? `<article class="search-directory-card collective-card chatroom-card"><b>${safe(item.name)}</b><span>${safe(item.description)}</span><small>${Number(item.members || 0)} collectors · ${(item.tags || []).map(tag => '#' + safe(tag)).join(' ')}</small><button type="button" data-chatroom="${safe(item.id)}" data-chatroom-label="${safe(item.name)}">Join voice room</button></article>` : `<article class="search-directory-card collective-card"><b>${safe(item.name)}</b><span>${safe(item.description)}</span><small>${Number(item.members || 0).toLocaleString()} members · ${Number(item.postCount || 0)} posts · ${(item.tags || []).map(tag => '#' + safe(tag)).join(' ')}</small></article>`;
