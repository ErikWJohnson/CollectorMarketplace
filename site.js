...
async function openAccountPanel() { if (!session?.user) return openAuthPanel(); const userId = session.user.id; const [profile, connections, suggestions] = await Promise.all([api(`/user/${userId}`).catch(() => ({ ...session.user, activeListings: listings.filter(item => item.ownerId === userId), tradeHistory: [] })), api(`/user/${userId}/connections`).catch(() => ({ friends: [], following: [] })), api('/users/suggestions').catch(() => [])]); openModal(`@${safe(profile.username)}`, 'Manage your collector profile and marketplace activity.', profileWorkspace(profile, { own: true, connections, suggestions })); modal.classList.add('profile-dialog', 'account-dialog'); document.body.classList.add('account-open'); }
async function openProfile(id) { if (!id) return; if (session?.user?.id === id) return openAccountPanel(); const profile = await api(`/user/${id}`); const following = Boolean(session?.user?.following?.includes(id)); openModal(`@${safe(profile.username)}`, `${safe(profile.bio || 'Collector profile')} · Reputation ${profile.reputation || 0}`, profileWorkspace(profile, { following })); modal.classList.add('profile-dialog'); document.body.classList.add('chat-open'); }
...
const collectorFeeRate = user => user?.curator === true || user?.membership === 'curator' ? .01 : .04;
