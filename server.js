...
app.post('/comment', required, (req, res) => { const { listingId, body, parentId } = req.body; const listing = store.data.listings.find(l => l.id === listingId); const parent = parentId ? store.data.comments.find(c => c.id === parentId) : null; const text = typeof body === 'string' ? body.trim() : ''; if (!listing || !text) return res.status(400).json({ error: 'Write a comment before posting.' }); if (text.length > 1000) return res.status(400).json({ error: 'Comments can be up to 1,000 characters.' }); if (parentId && (!parent || parent.listingId !== listingId)) return res.status(400).json({ error: 'Reply must belong to the same listing' }); const comment = { id: id(), listingId, userId: req.user.id, body: text, parentId: parentId || null, votes: {}, createdAt: now() }; store.data.comments.push(comment); const owner = parent ? parent.userId : listing.ownerId; if (owner && owner !== req.user.id) notify(owner, 'comment', `${req.user.username} commented on “${listing.title}”`, `/listing/${listing.id}`); activity('comment', req.user.id, { listingId, commentId: comment.id }); store.save(); res.status(201).json({ ...comment, score: 0, myVote: 0, user: publicUser(req.user) }); });
