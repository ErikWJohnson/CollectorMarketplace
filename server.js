...
app.put('/delivery/:id', required, (req, res) => {
  ...
  else if (delivery.sellerId === req.user.id && sellerStatuses.includes(status)) { const order = ['awaiting_seller_dispatch', 'packed', 'picked_up', 'in_transit']; const current = order.indexOf(delivery.status); const next = order.indexOf(status); if (next !== current + 1) return res.status(400).json({ error: 'Update the delivery one step at a time.' }); if (['picked_up', 'in_transit'].includes(status) && (!String(courier || delivery.courier).trim() || !String(trackingNumber || delivery.trackingNumber).trim())) return res.status(400).json({ error: 'Courier and tracking number are required once a package is picked up.' }); if (courier !== undefined) delivery.courier = String(courier).trim(); if (trackingNumber !== undefined) delivery.trackingNumber = String(trackingNumber).trim(); delivery.status = status; recordDeliveryUpdate(delivery, req.user.id, status, String(note).trim()); }
...
const rootSiteAssets = new Set([
  ...
  'listing-sort.css', 'delivery-workspace.css', 'app-performance.css', 'account-connections.css', 'favicon.png'
]);
