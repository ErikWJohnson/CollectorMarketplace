...
app.post('/delivery/:id/shippo/rate', required, async (req, res) => { try { const delivery = store.data.deliveries.find(row => row.id === req.params.id); if (!delivery || delivery.sellerId !== req.user.id) return res.status(404).json({ error: 'Delivery not found' }); const origin = shippoAddress(req.body.origin, 'sender'); const destination = shippoAddress(req.body.destination, 'recipient'); const parcel = shippoParcel(req.body.parcel); const shipment = await shippoRequest('/shipments/', { address_from: origin, address_to: destination, parcels: [parcel], async: false, metadata: `CollectorMarketplace ${delivery.id}` }); const rate = (shipment.rates || []).find(row => /ups/i.test(`${row.provider || ''} ${row.servicelevel?.name || ''}`)); if (!rate) return res.status(400).json({ error: 'Shippo returned no UPS rate for these details.' }); delivery.shippo = { shipmentId: shipment.object_id, rateId: rate.object_id, amount: Number(rate.amount), currency: rate.currency || 'USD', provider: rate.provider || 'UPS', service: rate.servicelevel?.name || 'UPS Priority', origin, destination, parcel }; recordDeliveryUpdate(delivery, req.user.id, delivery.status, `UPS rate quoted: ${delivery.shippo.currency} ${delivery.shippo.amount.toFixed(2)}.`); store.save(); res.json(deliveryView(delivery, req.user.id)); } catch (error) { res.status(400).json({ error: error.message }); } });
