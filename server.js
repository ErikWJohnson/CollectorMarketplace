...
app.post('/listing/:id/promotion', required, (req, res) => { const listing = store.data.listings.find(row => row.id === req.params.id); if (!listing) return res.status(404).json({ error: 'Listing not found' }); if (listing.ownerId !== req.user.id) return res.status(403).json({ error: 'Only the listing owner can promote this listing.' }); if (listing.status !== 'active') return res.status(400).json({ error: 'Restore this listing before promoting it.' }); const dailyBudget = Math.round(Number(req.body.dailyBudget) * 100) / 100; const durationDays = Math.round(Number(req.body.durationDays)); if (!Number.isFinite(dailyBudget) || dailyBudget < 1 || dailyBudget > 500) return res.status(400).json({ error: 'Choose a daily promotion budget between $1 and $500.' }); if (!Number.isFinite(durationDays) || durationDays < 1 || durationDays > 30) return res.status(400).json({ error: 'Choose a promotion duration between 1 and 30 days.' }); listing.promotion = { status: 'active', dailyBudget, durationDays, totalBudget: Math.round(dailyBudget * durationDays * 100) / 100, startedAt: now(), endsAt: new Date(Date.now() + durationDays * 86400000).toISOString(), billingStatus: 'test_mode_no_charge' }; activity('listing_promotion_started', req.user.id, { listingId: listing.id, dailyBudget, durationDays }); store.save(); res.json({ promotion: listing.promotion, statistics: listingStatistics(listing) }); });
