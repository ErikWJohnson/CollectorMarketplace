...
class Store {
  ...
  ensureData() { ['users', 'listings', 'comments', 'trades', 'notifications', 'activities', 'deliveries', 'conversations', 'communityPosts', 'memberships'].forEach(key => { if (!Array.isArray(this.data[key])) this.data[key] = []; }); this.data.users.forEach(user => { if (!Array.isArray(user.profileTags)) user.profileTags = []; if (String(user.username || '').trim().toLowerCase() === developerPassUsername) user.developerPass = true; }); const testAuction = this.data.listings.find(listing => listing.title === 'TEST' && listing.description === 'Internal payment-flow test listing. Do not purchase.'); if (testAuction) { testAuction.listingMode = 'auction_only'; testAuction.auctionEndless = true; testAuction.auctionEndAt = null; testAuction.auctionStartPrice = Number.isFinite(Number(testAuction.auctionStartPrice)) ? Number(testAuction.auctionStartPrice) : 0; } }
  load() {
...
const rootSiteAssets = new Set([
  ...
  'listing-sort.css', 'delivery-workspace.css', 'app-performance.css', 'account-connections.css', 'social-chat.css', 'voice-chat.css', 'account-workspace.css', 'tags-expanded.css', 'professional-ui.css', 'workspace-scrolling.css', 'scroll-stability.css', 'page-scroll-fixes.css', 'favicon.png', 'favicon-512.png', 'sitemap.xml', 'robots.txt', 'BingSiteAuth.xml'
]);
