...
app.post('/listing', required, (req, res) => { const { title, description, category, price, tradeOffer, images = [] } = req.body; const validImages = Array.isArray(images) && images.length > 0 && images.length <= 5 && images.every(image => typeof image === 'string' && image.length <= 2_000_000 && (/^https?:\/\//i.test(image) || /^data:image\/(jpeg|png|webp);base64,/i.test(image))); if (!title?.trim() || !description?.trim() || !category?.trim()) return res.status(400).json({ error: 'title, description, and category are required' }); if (!validImages) return res.status(400).json({ error: 'Add 1–5 valid image links or uploads.' }); const listing = { id: id(), ownerId: req.user.id, title: title.trim(), description: description.trim(), category: category.trim(), price: Number(price) || 0, tradeOffer: Boolean(tradeOffer), images, status: 'active', likes: [], createdAt: now() }; store.data.listings.unshift(listing); activity('listing', req.user.id, { listingId: listing.id }); store.save(); res.status(201).json(listing); });
