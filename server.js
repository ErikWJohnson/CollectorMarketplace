...
app.post('/listing', required, (req, res) => { const { title, description, category, tags = [], price, tradeOffer, images = [], videos = [], location } = req.body; const validImages = Array.isArray(images) && images.length > 0 && images.length <= 5 && images.every(image => typeof image === 'string' && image.length <= 2_000_000 && (/^https?:\/\//i.test(image) || /^data:image\/(jpeg|png|webp);base64,/i.test(image))); const validVideos = Array.isArray(videos) && videos.length <= 1 && videos.every(video => typeof video === 'string' && video.length <= 6_000_000 && /^data:video\/(mp4|webm|quicktime);base64,/i.test(video)); if (!title?.trim() || !description?.trim() || !category?.trim()) return res.status(400).json({ error: 'title, description, and category are required' }); const publicLocation = typeof location === 'string' ? location.trim() : ''; if (publicLocation.length < 2 || publicLocation.length > 120) return res.status(400).json({ error: 'Add a public city/region location (2–120 characters).' }); if (/^\d{1,6}\s+|\b(street|st\.?|avenue|ave\.?|road|rd\.?|boulevard|blvd\.?|drive|dr\.?|apartment|apt\.?|suite|unit|zip)\b/i.test(publicLocation)) return res.status(400).json({ error: 'Use a city, region, and country only—do not post a street address.' }); const normalizedTags = [...new Set([category.trim(), ...(Array.isArray(tags) ? tags : []).map(tag => typeof tag === 'string' ? tag.trim().replace(/^#/, '') : '').filter(Boolean)])]; const validTags = normalizedTags.length <= 8 && normalizedTags.every(tag => tag.length <= 60); if (!validTags) return res.status(400).json({ error: 'Use up to 8 tags, each 60 characters or less.' }); if (!validImages) return res.status(400).json({ error: 'Add 1–5 valid image links or uploads.' }); if (!validVideos) return res.status(400).json({ error: 'Add at most one valid uploaded video.' }); const listing = { id: id(), ownerId: req.user.id, title: title.trim(), description: description.trim(), category: category.trim(), tags: normalizedTags, location: publicLocation, price: Number(price) || 0, tradeOffer: Boolean(tradeOffer), images, videos, status: 'active', likes: [], createdAt: now() }; store.data.listings.unshift(listing); activity('listing', req.user.id, { listingId: listing.id }); store.save(); res.status(201).json(listing); });
