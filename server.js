...
function recordUniqueListingMetric(listing, req, metric) { const visitor = uniqueListingVisitor(req, metric); if (!visitor) return false; const visitorsKey = metric === 'impression' ? 'impressionVisitors' : 'viewVisitors'; const countKey = metric === 'impression' ? 'impressionCount' : 'viewCount'; if (!listing[visitorsKey] || typeof listing[visitorsKey] !== 'object' || Array.isArray(listing[visitorsKey])) listing[visitorsKey] = {}; if (listing[visitorsKey][visitor]) return false; listing[visitorsKey][visitor] = now(); listing[countKey] = Number(listing[countKey] || 0) + 1; return true; }
...
const listingStatistics = (listing, includePrivateAudience = false) => { const votes = listing.votes && typeof listing.votes === 'object' ? listing.votes : {}; const voterIds = [...new Set([...(listing.likes || []), ...Object.keys(votes)])]; const voter = userId => { const user = store.data.users.find(candidate => candidate.id === userId); return user ? { id: user.id, username: user.username, avatar: user.avatar || '', profileTags: user.profileTags || [] } : null; }; const audienceTags = ['Male', 'Female', 'Unisex', 'LGTBQ+', 'Baby', 'Toddler', 'Child', 'Teen', 'Adult', 'Elder']; const selfDeclaredAudience = Object.fromEntries(audienceTags.map(tag => [tag, voterIds.filter(userId => (store.data.users.find(user => user.id === userId)?.profileTags || []).some(profileTag => String(profileTag).toLowerCase() === tag.toLowerCase())).length])); const completedSales = store.data.deliveries.filter(delivery => delivery.listingId === listing.id && delivery.status === 'completed' && !delivery.tradeId && delivery.paypal?.status === 'COMPLETED'); const grossSales = completedSales.reduce((total, delivery) => total + Number(delivery.itemPrice || 0), 0); const sellerFees = completedSales.reduce((total, delivery) => total + Number(delivery.fees?.seller?.amount || 0), 0); const promotionSpend = (store.data.promotions || []).filter(promotion => promotion.listingId === listing.id && promotion.paypal?.status === 'COMPLETED').reduce((total, promotion) => total + Number(promotion.totalBudget || 0), 0); const statistics = { impressions: Number(listing.impressionCount || 0), views: Number(listing.viewCount || 0), saves: Array.isArray(listing.favorites) ? listing.favorites.length : 0, likes: Array.isArray(listing.likes) ? listing.likes.length : 0, comments: store.data.comments.filter(comment => comment.listingId === listing.id).length, sales: completedSales.length, grossSales: Math.round(grossSales * 100) / 100, sellerFees: Math.round(sellerFees * 100) / 100, promotionSpend: Math.round(promotionSpend * 100) / 100, profit: Math.round((grossSales - sellerFees - promotionSpend) * 100) / 100, stockQuantity: Number(listing.stockQuantity || 1), stockRemaining: Number.isInteger(listing.stockRemaining) ? listing.stockRemaining : 1, tradeInquiries: (store.data.trades || []).filter(trade => trade.listingId === listing.id || (trade.receiverListingIds || []).includes(listing.id) || (trade.senderListingIds || []).includes(listing.id)).length, voteScore: Object.values(votes).reduce((total, value) => total + Number(value || 0), 0), promotion: listing.promotion || null }; if (includePrivateAudience) statistics.audience = { sampleSize: voterIds.length, selfDeclaredTags: selfDeclaredAudience, location: 'Not collected for advertising analytics', upvoters: Object.entries(votes).filter(([, value]) => Number(value) === 1).map(([userId]) => voter(userId)).filter(Boolean), downvoters: Object.entries(votes).filter(([, value]) => Number(value) === -1).map(([userId]) => voter(userId)).filter(Boolean) }; return statistics; };
...
app.put('/user/:id', required, (req, res) => { if (req.user.id !== req.params.id) return res.status(403).json({ error: 'Not allowed' }); ['username','avatar','bio'].forEach(k => { if (req.body[k] !== undefined) req.user[k] = req.body[k]; }); if (req.body.lobbySong !== undefined) { const song = String(req.body.lobbySong || ''); if (song && (!/^data:audio\/(mpeg|mp3)(?:;[a-z0-9=._-]+)*;base64,[a-z0-9+/=]+$/i.test(song) || song.length > 5500000)) return res.status(400).json({ error: 'Use an MP3 lobby song under 4 MB.' }); req.user.lobbySong = song; } if (req.body.profileTags !== undefined) { if (!Array.isArray(req.body.profileTags)) return res.status(400).json({ error: 'Profile tags must be a list.' }); const tags = [...new Set(req.body.profileTags.map(tag => String(tag).replace(/^#/, '').trim()).filter(tag => tag && tag.length <= 48))].slice(0, 20); req.user.profileTags = tags; } if (req.body.story !== undefined) { const story = String(req.body.story || '').trim(); if (story.length > 5000) return res.status(400).json({ error: 'My Story can be up to 5,000 characters.' }); req.user.story = story; } if (req.body.website !== undefined) { const website = String(req.body.website || '').trim(); if (website && (!/^https:\/\/[^\s]{1,200}$/i.test(website))) return res.status(400).json({ error: 'Use a valid HTTPS website URL.' }); req.user.website = website; } if (req.body.socialLinks !== undefined) { if (!Array.isArray(req.body.socialLinks)) return res.status(400).json({ error: 'Links must be a list.' }); const links = req.body.socialLinks.map(link => ({ label: String(link?.label || '').trim(), url: String(link?.url || '').trim() })).filter(link => link.label || link.url); if (links.length > 6 || links.some(link => !link.label || link.label.length > 40 || !/^https:\/\/[^\s]{1,200}$/i.test(link.url))) return res.status(400).json({ error: 'Add up to six named HTTPS links.' }); req.user.socialLinks = links; } activity('profile', req.user.id); store.save(); res.json({ ...publicUser(req.user), lobbySong: req.user.lobbySong || '' }); });
...
app.get('/listing/:id', (req, res) => { const listing = store.data.listings.find(l => l.id === req.params.id); if (!listing) return res.status(404).json({ error: 'Listing not found' }); if (recordUniqueListingMetric(listing, req, 'view')) store.save(); res.json({ ...publicListing(listing), owner: publicUser(store.data.users.find(u => u.id === listing.ownerId)), likeCount: listing.likes.length, statistics: listingStatistics(listing) }); });
...
app.put('/listing/:id', required, (req, res) => {
  ...
  if (req.body.stockQuantity !== undefined) {
    ...
    const soldUnits = Math.max(0, Number(listing.stockQuantity || 1) - Number(listing.stockRemaining ?? listing.stockQuantity ?? 1));
    if (quantity < soldUnits) return res.status(400).json({ error: `Stock cannot be lower than the ${soldUnits} unit${soldUnits === 1 ? '' : 's'} already sold.` });
    listing.stockQuantity = quantity;
    listing.stockRemaining = quantity - soldUnits;
    if (listing.stockRemaining > 0 && listing.status === 'sold') listing.status = 'active';
  }
  ...
  if (Number(listing.stockQuantity || 1) > 1) listing.tradeOffer = false;
...
app.post('/paypal/orders', required, async (req, res) => {
  ...
  try {
    ...
    if (delivery) { store.data.deliveries = store.data.deliveries.filter(row => row.id !== delivery.id); store.save(); }
...
async function capturePayPalDelivery(delivery) {
  ...
  delivery.status = 'awaiting_seller_dispatch'; const listing = store.data.listings.find(row => row.id === delivery.listingId);
  if (listing) {
    const remaining = Number.isInteger(listing.stockRemaining) ? listing.stockRemaining : 1;
    if (remaining < 1) throw new Error('This listing is out of stock.');
    listing.stockRemaining = remaining - 1;
    listing.status = listing.stockRemaining > 0 ? 'active' : 'pending_delivery';
  }
...
function cancelPayPalDelivery(delivery) {
  ...
  delivery.status = 'payment_cancelled'; delivery.paypal = { ...delivery.paypal, status: 'CANCELLED' }; recordDeliveryUpdate(delivery, delivery.buyerId, delivery.status, `Buyer cancelled ${paypalLabel()} checkout.`); store.save();
}
...
app.post('/delivery/:id/return-received', required, (req, res) => {
  ...
  const listing = store.data.listings.find(row => row.id === delivery.listingId); if (listing && !delivery.tradeId) { listing.stockRemaining = Math.min(Number(listing.stockQuantity || 1), Math.max(0, Number(listing.stockRemaining || 0)) + 1); listing.status = 'active'; }
