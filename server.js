...
app.post('/listing', required, (req, res) => {
  ...
  if (!['pickup', 'delivery', 'both'].includes(fulfillment)) return res.status(400).json({ error: 'Choose pickup, delivery, or both for fulfillment.' });
...
app.post('/purchase', required, (req, res) => {
  ...
  if (listing.fulfillment === 'delivery' && provider === 'Local pickup') return res.status(400).json({ error: 'This listing is available for delivery only.' });
