...
function publicUser(user) { if (!user) return null; const { password, email, shippingProfile, ...safe } = user; return safe; }
function directoryUser(user) { if (!user) return null; const { password, email, following, shippingProfile, ...safe } = user; return safe; }
...
app.put('/user/:id', required, (req, res) => { if (req.user.id !== req.params.id) return res.status(403).json({ error: 'Not allowed' }); ['username','avatar','bio'].forEach(k => { if (req.body[k] !== undefined) req.user[k] = req.body[k]; }); if (req.body.profileTags !== undefined) { if (!Array.isArray(req.body.profileTags)) return res.status(400).json({ error: 'Profile tags must be a list.' }); const tags = [...new Set(req.body.profileTags.map(tag => String(tag).replace(/^#/, '').trim()).filter(tag => tag && tag.length <= 48))].slice(0, 20); req.user.profileTags = tags; } activity('profile', req.user.id); store.save(); res.json(publicUser(req.user)); });
...
app.get('/listings', (req, res) => { const { category, q, status = 'active' } = req.query; let rows = store.data.listings.filter(l => !status || l.status === status); if (category) rows = rows.filter(l => l.category === category); if (q) { const s = q.toLowerCase(); rows = rows.filter(l => `${l.title} ${l.description} ${l.category || ''} ${l.condition || ''} ${(l.tags || []).join(' ')}`.toLowerCase().includes(s)); } res.json(rows.map(l => ({ ...l, owner: publicUser(store.data.users.find(u => u.id === l.ownerId)), likeCount: l.likes.length, commentCount: store.data.comments.filter(comment => comment.listingId === l.id).length }))); });
...
app.post('/comment', required, (req, res) => { const { listingId, body, parentId, mediaUrl } = req.body; const listing = store.data.listings.find(l => l.id === listingId); const parent = parentId ? store.data.comments.find(c => c.id === parentId) : null; const text = typeof body === 'string' ? body.trim() : ''; const image = typeof mediaUrl === 'string' ? mediaUrl.trim() : ''; if (!listing || !text) return res.status(400).json({ error: 'Write a comment before posting.' }); if (text.length > 1000) return res.status(400).json({ error: 'Comments can be up to 1,000 characters.' }); if (image && (!/^https:\/\/[\w.-]+\//i.test(image) || image.length > 2000)) return res.status(400).json({ error: 'Use a valid HTTPS image link under 2,000 characters.' }); if (parentId && (!parent || parent.listingId !== listingId)) return res.status(400).json({ error: 'Reply must belong to the same listing' }); const comment = { id: id(), listingId, userId: req.user.id, body: text, mediaUrl: image || '', parentId: parentId || null, votes: {}, createdAt: now() }; store.data.comments.push(comment); const owner = parent ? parent.userId : listing.ownerId; if (owner && owner !== req.user.id) notify(owner, 'comment', `${req.user.username} commented on “${listing.title}”`, `/listing/${listing.id}`); activity('comment', req.user.id, { listingId, commentId: comment.id }); store.save(); res.status(201).json({ ...comment, score: 0, myVote: 0, user: publicUser(req.user) }); });
...
const rootSiteAssets = new Set([
  ...
  'listing-sort.css', 'delivery-workspace.css', 'app-performance.css', 'account-connections.css', 'social-chat.css', 'voice-chat.css', 'account-workspace.css', 'tags-expanded.css', 'professional-ui.css', 'workspace-scrolling.css', 'scroll-stability.css', 'favicon.png', 'favicon-512.png', 'sitemap.xml', 'robots.txt', 'BingSiteAuth.xml'
]);
