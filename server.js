...
app.post('/listing', required, (req, res) => {
  const { title, description, category, tags = [], price, tradeOffer, images = [], videos = [], location, locationCoordinates } = req.body;
  ...
  const listing = { id: id(), ownerId: req.user.id, title: title.trim(), description: description.trim(), category: category.trim(), tags: normalizedTags, location: publicLocation, locationCoordinates: coordinates, price: Number(price) || 0, tradeOffer: Boolean(tradeOffer), images, videos, status: 'active', likes: [], createdAt: now() };
